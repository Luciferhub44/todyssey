import { hexlify, keccak256 } from "ethers/utils";
...
export default class MerkleTree {
  ...
  static combinedHash(first: Buffer, second: Buffer): Buffer {
    ...
    return Buffer.from(
      keccak256(MerkleTree.sortAndConcat(first, second)),
      "hex"
    );
  }
  ...
  getHexRoot(): string {
    return hexlify(this.getRoot());
  }

  getProof(el: Buffer) {
    let idx = this.bufferElementPositionIndex[hexlify(el)];
